...
def load_dataset(dataset,
    ...
    if dataset == 'lenta':
        ...
        if max_docs_cnt is not None:
            ...
            max_docs_cnt = int(docs_cnt * max_docs_frac)
            print(f"Restricted to {max_docs_cnt} documents")
        else:
            ...
            if w[0]:
                if delete_last_sentence and len(w[0]) > len(w[1]):
                    ...
                    train_data_list.append(
                        (new_w0, expand_label(new_labs, new_w0)))
        ...
        data = WikipediaDataSet('data/wiki_test_50',
                                folder=True, only_letters=False)
        data_list = []
        for w in data:
        ...
        for root, direct, files in os.walk(file_dir):
            for file in files:
                ...
                try:
                    seg_file = [
                        x for x in segment_files if re.search(file[:-6], x)][0]
                ...
                tmp = pd.DataFrame(df.iloc[:, 0].str.split(
                    '_').tolist(), columns=['id', 'start', 'end'])
                ...
                for topic in topic_list:
                    ...
                    if len(text_spans) > 1:
                        if ct not in remainder_topics:
                            remainder_topics[ct] = set(
                                int(t[1]) for t in text_spans[1:])
                        else:
                            remainder_topics[ct] = remainder_topics[ct].union(
                                set(int(t[1]) for t in text_spans[1:]))
                    ...
                    if int(seg[0])-1 > start:
                        ...
                        prev_text = file["meeting_transcripts"][start:int(
                            seg[0])]
                    ...
                    text = file["meeting_transcripts"][int(
                        seg[0]):int(seg[1])+1]
                    popped = 0
                    if mask_inner_sentences:
