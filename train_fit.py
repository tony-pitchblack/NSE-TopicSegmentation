...
def main(args):
    ...
    if args.architecture.lower().startswith('text'):
        ...
        if args.vq_bert_encoder:
            encoder = get_topic_bert_combined_model(
                args.vq_path, args.encoder, max_length=args.max_length, labels=args.zero_shot_labels)
        elif args.vq_encoder:
            encoder = get_vq_model(args.vq_path)
        else:
            encoder = get_model(args.encoder, max_length=args.max_length,
                                labels=args.zero_shot_labels, emb_diff=args.embedding_difference)[0]
        if isinstance(args.second_encoder, str):
            second_encoder = get_model(args.second_encoder, max_length=args.max_length,
                                       labels=args.zero_shot_labels, emb_diff=args.embedding_difference)[0]
    ...
    if args.architecture.lower().startswith('text'):
        ...
        for fold in folds:
            if not test:  # switch to change the validation/training split
                ...
                train_dataset = WordLevelDataset(
                    fold[0], tag_to_ix, word2index, WordMatrix)
                valid_dataset = WordLevelDataset(
                    fold[2], tag_to_ix, word2index, WordMatrix)
                ...
                train_dataset = WordLevelDataset(
                    train_samples, tag_to_ix, word2index, WordMatrix)
                valid_dataset = WordLevelDataset(
                    valid_samples, tag_to_ix, word2index, WordMatrix)

            test_dataset = WordLevelDataset(
                fold[1], tag_to_ix, word2index, WordMatrix)
            train_loader = DataLoader(train_dataset, batch_size=min(
                bs, len(train_dataset)), collate_fn=train_dataset.collater)
            if verbose:
                print('Train loader has: {} documents'.format(len(train_dataset)))
            valid_loader = DataLoader(valid_dataset, batch_size=min(
                bs, len(valid_dataset)), collate_fn=valid_dataset.collater)
            if verbose:
                print('Validation loader has: {} documents'.format(
                    len(valid_dataset)))
            test_loader = DataLoader(test_dataset, batch_size=min(
                bs, len(test_dataset)), collate_fn=valid_dataset.collater)
            if verbose:
        ...
        for fold in folds:
            if not test:  # switch to change the validation/training split
                ...
                # qmsum is the only dataset having a fixed development set
                train_dataset = CrossEncoderDataset(
                    fold[0], enc=encoder, minus=CRF, longformer=False)
                valid_dataset = CrossEncoderDataset(
                    fold[2], enc=encoder, minus=CRF, longformer=False)
                ...
                train_dataset = CrossEncoderDataset(
                    train_samples, enc=encoder, minus=CRF, longformer=False)
                valid_dataset = CrossEncoderDataset(
                    valid_samples, enc=encoder, minus=CRF, longformer=False)

            test_dataset = CrossEncoderDataset(
                fold[1], enc=encoder, minus=CRF, longformer=False)
            train_loader = DataLoader(train_dataset, batch_size=min(
                bs, len(train_dataset)), collate_fn=train_dataset.collater)
            if verbose:
                print('Train loader has: {} documents'.format(len(train_dataset)))
        ...
        for fold in folds:
            if not test:  # switch to change the validation/training split
                ...
                valid_dataset = SentenceDataset(fold[0][-valid_split:], tag_to_ix, encoder=encoder, precompute=precompute,
                                                CRF=CRF, manual_max_length=args.max_doc_length, second_encoder=second_encoder)

            elif args.dataset.lower() == 'qmsum' or args.dataset.startswith('wikisection'):
                # qmsum is the only dataset having a fixed development set
                train_dataset = SentenceDataset(fold[0], tag_to_ix, encoder=encoder, precompute=precompute, CRF=CRF, cosine_loss=args.cosine_loss, manual_max_length=args.max_doc_length,
                                                mask_inner_sentences=args.mask_negatives_in_sequence, mask_probability=args.mask_percentage, second_encoder=second_encoder)
                valid_dataset = SentenceDataset(fold[2], tag_to_ix, encoder=encoder, precompute=precompute,
                                                CRF=CRF, manual_max_length=args.max_doc_length, second_encoder=second_encoder)
                ...
                train_dataset = SentenceDataset(train_samples, tag_to_ix, encoder=encoder, precompute=precompute, CRF=CRF, cosine_loss=args.cosine_loss,
                                                manual_max_length=args.max_doc_length, mask_inner_sentences=args.mask_negatives_in_sequence, mask_probability=args.mask_percentage, second_encoder=second_encoder)
                valid_dataset = SentenceDataset(valid_samples, tag_to_ix, encoder=encoder, precompute=precompute,
                                                CRF=CRF, manual_max_length=args.max_doc_length, second_encoder=second_encoder)

            test_dataset = SentenceDataset(fold[1], tag_to_ix, encoder=encoder, precompute=precompute,
                                           CRF=CRF, manual_max_length=args.max_doc_length, second_encoder=second_encoder)
            train_loader = DataLoader(train_dataset, batch_size=min(
                bs, len(train_dataset)), collate_fn=train_dataset.collater)
            if verbose:
        ...
        if test:
            train_embeddings = load_embeddings(
                args.encoder + '_train', args.dataset, args.embeddings_directory)
            valid_embeddings = load_embeddings(
                args.encoder + '_valid', args.dataset, args.embeddings_directory)
            test_embeddings = load_embeddings(
                args.encoder + '_test', args.dataset, args.embeddings_directory)

            for fold in folds:
                if args.dataset.lower() == 'qmsum' or args.dataset.startswith('wikisection'):
                    train_dataset = SentenceDataset(fold[0], tag_to_ix, encoder=encoder, embeddings=train_embeddings, CRF=CRF, cosine_loss=args.cosine_loss,
                    ...
                    valid_dataset = SentenceDataset(fold[0][-valid_split:], tag_to_ix, encoder=encoder,
                                                    embeddings=valid_embeddings, CRF=CRF, manual_max_length=args.max_doc_length)
                test_dataset = SentenceDataset(
                    fold[1], tag_to_ix, encoder=encoder, embeddings=test_embeddings, CRF=CRF, manual_max_length=args.max_doc_length)
                train_loader = DataLoader(train_dataset, batch_size=min(
                    bs, len(train_dataset)), collate_fn=train_dataset.collater)
                if verbose:
                    print('Train loader has: {} documents'.format(
                        len(train_dataset)))
                valid_loader = DataLoader(valid_dataset, batch_size=min(
                    bs, len(valid_dataset)), collate_fn=valid_dataset.collater)
                if verbose:
            ...
            for index, fold in enumerate(folds):
                ...
                train_dataset = SentenceDataset(fold[0][:-valid_split], tag_to_ix, encoder=encoder, embeddings=embeddings[index][0][:-valid_split], CRF=CRF, cosine_loss=args.cosine_loss,
                                                manual_max_length=args.max_doc_length, mask_inner_sentences=args.mask_negatives_in_sequence, mask_probability=args.mask_percentage)
                valid_dataset = SentenceDataset(fold[0][-valid_split:], tag_to_ix, encoder=encoder,
                                                embeddings=embeddings[index][0][-valid_split:], CRF=CRF, manual_max_length=args.max_doc_length)
                test_dataset = SentenceDataset(
                    fold[1], tag_to_ix, encoder=encoder, embeddings=embeddings[index][1], CRF=CRF, manual_max_length=args.max_doc_length)
                train_loader = DataLoader(train_dataset, batch_size=min(
                    bs, len(train_dataset)), collate_fn=train_dataset.collater)
                if verbose:
                    print('Train loader has: {} documents'.format(
    ...
    for param_index, param_tuple in enumerate(hyperparameters):
        ...
        for index, segm in enumerate(loaders):
            ...
            threshold = args.threshold if args.threshold else float(
                checkpoint_callback.best_model_path.split('=')[-1][:4])

            valid_loss = float(
                checkpoint_callback.best_model_path.split('=')[-2][:4])
            results_valid.append(valid_loss)

            if args.save_last_epoch:
                trainer.save_checkpoint(
                    os.path.join(check_dir, "final=0.50.ckpt"))
                checkpoint_callback.best_model_path = os.path.join(
                    check_dir, "final=0.50.ckpt")
                threshold = 0.5
            ...
            if args.metric == 'B' or args.metric == 'scaiano':
                with open('logs', 'a') as f:
                    ...
                    f.write('B_recall score: {}\n'.format(
                        results[-1][0][wd_label]))
                    f.write('B_F1 score: {}\n'.format(
                        results[-1][0][f1_label]))
            ...
            if f1_best or (args.metric == 'Pk' and Valid < best_results) or (args.metric == 'WD' and Valid < best_results):
                ...
                def bootstrap(data, samples=10000):
                    ...
                    for sample in range(samples):
                        boot.append(data.sample(
                            len(data), replace=True).mean()[0])
                    return boot

                boots = bootstrap(Pks)

                confidence_Pks = (np.percentile(
                    boots, 97.5) - np.percentile(boots, 2.5))/2

                boots = bootstrap(F1s)

                confidence_F1s = (np.percentile(
                    boots, 97.5) - np.percentile(boots, 2.5))/2

                boots = bootstrap(WDs)

                confidence_WDs = (np.percentile(
                    boots, 97.5) - np.percentile(boots, 2.5))/2

                if args.metric == 'B':
                    boots = bootstrap(Bs)
                    confidence_Bs = (np.percentile(
                        boots, 97.5) - np.percentile(boots, 2.5))/2
    ...
    if args.save_embeddings:
        if test:
            train_dataset.save_embeddings(
                args.encoder + '_train', args.dataset)
            valid_dataset.save_embeddings(
                args.encoder + '_valid', args.dataset)
            test_dataset.save_embeddings(args.encoder + '_test', args.dataset)
        else:
            train_dataset.embeddings = train_dataset.embeddings.extend(
                valid_dataset.embeddings)
            train_dataset.embeddings = train_dataset.embeddings.extend(
                test_dataset.embeddings)
    ...
    if test:
        ...
        output = ['Results for experiment {} with following parameters:'.format(args.experiment_name),
                  ...
                  'Mean {} obtained is {}'.format(
                      label_map['Pk'], best_results_test['Pk']),
                  'Mean F1 obtained is {}'.format(best_results_test['F1']),
                  'Mean {} obtained is {}'.format(
                      label_map['WD'], best_results_test['WD']),
                  'Best Validation Loss: {}'.format(best_results)]

        if args.metric == 'B':
            output.append('Mean Boundary Similarity obtained is {}'.format(
                best_results_test['B']))
        ...
        output = ['Results for experiment {} with following parameters:'.format(args.experiment_name),
                  ...
                  'Number of layers: {}'.format(best_nl),
                  'Optimizer: {}'.format(args.optimizer),
                  'Mean {} obtained is {} with a 95% confidence interval of +- {}'.format(
                      label_map['Pk'], best_results_test['Pk'], confidence_Pks),
                  'Mean F1 obtained is {} with a 95% confidence interval of +- {}'.format(
                      best_results_test['F1'], confidence_F1s),
                  'Mean {} obtained is {} with a 95% confidence interval of +- {}'.format(
                      label_map['WD'], best_results_test['WD'], confidence_WDs),
                  'Best Validation Loss: {} with a 95% confidence interval of +- {}'.format(best_results, confidence_Valid)]
        if args.metric == 'B':
            output.append('Mean Boundary Similarity obtained is {} with a 95% confidence interval of +- {}'.format(
                best_results['B'], confidence_Bs))
        if args.zero_shot_labels is not None:
            output.append('Labels: ' + str(args.zero_shot_labels))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_end_boundary', '-ueb', action='store_true',
                        help='Whether if to include the final sentence as a positive target in training (it will not be included when computing the metrics, but just for having additional positive classes in training)')
    ...
    parser.add_argument('--search_threshold', '-sth', action='store_true',
                        help='Whether to search for the optimal threshold during training')

    parser.add_argument('--cosine_loss', '-cos', action='store_true',
                        help="Whether to include the cosine loss of the last layers' embeddings as an additional loss.")

    parser.add_argument('--gradient_clipping', '-gc', default=0.0,
                        type=float, help='The value to clip the gradient to.')

    parser.add_argument('--restricted', '-rsa', action='store_true',
                        help='If using transformer, this option allows for using restricted self-attention instead of global self-attention, with a window parameter specified below.')

    parser.add_argument('--window_size', '-ws', default=3, type=int,
                        help='The window parameter for the restricted self-attention mechanism (see above)')

    parser.add_argument('--sixteen_bit', '-16b', action='store_true',
                        help='Enable training in 16 bits, making trainer faster and less memory intensive (performance might drop)')
    ...
    parser.add_argument('--loss_function', '-loss', choices=["CrossEntropy", "BinaryCrossEntropy", "FocalLoss"], default="CrossEntropy",
                        help="The loss function to minimise during training. Available options are CrossEntropy that treat the problem as a two class problem and BinaryCrossEntropy that treats the problem as a binary classification task (in this case an additional sigmoid is applied to the output of the model)")

    parser.add_argument('--vq_encoder', '-vqe', action='store_true',
                        help='Whether to use the vector quantisation model as sentence encoder.')

    parser.add_argument('--vq_path', '-vqp', default='VQVAE_BBC', type=str,
                        help='path to the pre-trained vector quantisation model components')

    parser.add_argument('--vq_bert_encoder', '-vbe', action='store_true',
                        help='Use both pre-trained sentence encoder and vector quantisation encoder to produce sentence embeddings. The sentence encoder used is defined by the --encoder argument, while the path to the vq model is given by the --vq_path argument. This option overrides the --vq_encoder one.')

    parser.add_argument('--auxiliary_coherence_original', '-aux', action='store_true',
                        help='Add the coherence objective defined in Xing et al., 2020')

    parser.add_argument('--restricted_attention', '-ra', action='store_true',
                        help='Add the restricted attention mechanism as defined in Xing et al., 2020')

    parser.add_argument('--wandb', action='store_true',
                        help='Use wandb as a logger')

    parser.add_argument('--wandb_key', default="0", type=str,
                        help="If using wandb, you can directly provide your api key with your option to avoid been prompted to insert the key (good, for example, if you're submitting the script as a job on a cluster)")

    parser.add_argument('--mask_negatives', action='store_true',
                        help='Mask a percentage of negatives examples to tackle the label unbalance problem. (DEPRECATED)')

    parser.add_argument('--mask_negatives_in_sequence', '-mask', action='store_true',
                        help='Mask a percentage of negatives examples to tackle the label unbalance problem. Unlike the option above, by using this option the masking involve just ignoring the masked sentences in the loss function, therefore allowing their information to be modelled by the model anyway.')

    parser.add_argument('--mask_percentage', default=0.7, type=float,
                        help='In case of using a mask for negative examples (see above), this parameter specifies the percentage of such examples to be left out.')

    parser.add_argument('--embedding_difference', '-diff', action='store_true',
                        help='Whether to use the difference between adjacent embeddings as the feature for the segmentation system.')

    parser.add_argument('--save_test_scores', action='store_true',
                        help='If included, with this option the test scores are saved and written in the results folder, so that alternative forms of testing can then be employed.')

    parser.add_argument('--max_doc_length', required=False, type=int,
                        help="If included it manually gives the number of sentences to which pad/truncate documents.")

    parser.add_argument('--limit_train_batches', default=1.0,
                        help="An option to specify how many batches per training epoch to process. This option is included to replicate the setting from Kelvin et al. 2021, where they process 100 batches per epoch.")

    parser.add_argument('--limit_valid_batches', default=1.0,
                        help="An option to specify how many batches per validation epoch to process. This option is included to replicate the setting from Kelvin et al. 2021, where they process 100 batches per epoch.")

    parser.add_argument('--save_last_epoch', '-s_last', action='store_true',
                        help="If included, use the model from last training epoch as checkpoint instead of the best validation one.")
