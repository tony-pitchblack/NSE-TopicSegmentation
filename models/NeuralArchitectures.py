...
def create_mask(src, lengths):
    ...
    for index, i in enumerate(src):
        ...
        mask.append([False if (i) < lengths[index]
                    else True for i in range(max_len)])
...
class RNN(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, num_layers=1, labels=1,
        ...
        if self.bidirectional:
            ...
            # option to perform multitask learning with polarity of the sentence
            self.polarity = nn.Linear(hidden_size*2, 2)
            # option to perform multitask learning with arousal of the sentence
            self.arousal = nn.Linear(hidden_size*2, 4)
        else:
            self.output = nn.Linear(hidden_size, labels)
            # option to perform multitask learning with polarity of the sentence
            self.polarity = nn.Linear(hidden_size, 2)
            # option to perform multitask learning with arousal of the sentence
    ...
    def forward(self, line, line_len=None, apply_softmax=False, return_final=False, classifier=False):
        ...
        if line_len is not None:
            ...
            embedded = PACK(line, line_len.data.tolist(),
        ...
        if self.bidirectional:
            ...
            if line_len is not None:
                ...
                rnn_out_new = rnn_out.contiguous().view(
                    batch_size, seq_size, feat_size)[:, -1]
...
class Transformer(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, n_heads, n_layers, dropout=0.2, drop_out=0.0, batch_first=True, device='cuda', positional_encoding=True, restricted=False, window_size=None, pyramidal=False):
        ...
        if pyramidal:
            # batch_first = True is super slow. Try to change the mask and the input instead
            encoder_layers = [RestrictedTransformerEncoderLayer(
                in_dim, n_heads, h_dim, win*window_size, dropout, batch_first=True) for win in range(n_layers, 1, -1)]
        elif restricted:
            encoder_layers = RestrictedTransformerEncoderLayer(
                in_dim, n_heads, h_dim, window_size, dropout, batch_first=True)
        else:
            encoder_layers = nn.TransformerEncoderLayer(
                in_dim, n_heads, h_dim, dropout, batch_first=True)

        if pyramidal:
            # self.transformer_encoder = PyramidalTransformerEncoder(encoder_layers, norm=nn.LayerNorm(in_dim))
            self.transformer_encoder = PyramidalTransformerEncoder(
                encoder_layers, norm=None)
        else:
            self.transformer_encoder = nn.TransformerEncoder(
                encoder_layers, n_layers, norm=nn.LayerNorm(in_dim))
...
class Convolutional(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, n_layers, dropout=0.2, drop_out=0.0, kernel=1):
        ...
        net = OrderedDict([('conv0', nn.Conv1d(
            in_dim, h_dim, kernel, padding='same')), ('activation0', nn.ReLU())])
