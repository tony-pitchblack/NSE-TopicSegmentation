...
def load_embeddings(encoder_name, dataset_name, parent_directory='../embeddings'):
    ...
    assert os.path.exists(
        in_dir), "The directory provided {} for the embeddings does not exist!".format(in_dir)
    ...
    embeddings = [
        embedding for embedding in embeddings if not isinstance(embedding, int)]
...
class SentenceDataset(Dataset):
    def __init__(self, lines, tag_to_ix, encoder='stsb-bert-base',
        ...
        if infer:
            ...
            self.tgt_dataset = [[tag_to_ix[tag]
                                 for tag in line[1]] for line in lines]
            ...
            if precompute:
                ...
                if second_encoder is not None:
                    second_embeddings = [self.encoder2.encode(
                        sents, convert_to_tensor=True, show_progress_bar=False).detach().cpu() for sents in self.sentences]
                    print("Finished encoding with the second encoder!")
                    # if pre-computing, it is useless to keep the encoder in the dataset object after encoding is done
                    self.encoder2 = None
                    self.embeddings = [torch.cat(
                        (emb, second_embeddings[i]), axis=1) for i, emb in enumerate(self.embeddings)]
                # if pre-computing, it is useless to keep the encoder in the dataset object after encoding is done
                self.encoder = None
    ...
    def from_precomputed(cls, lines, tag_to_ix, encoder_name, dataset_name, parent_directory='embeddings'):
        embeddings = load_embeddings(
            encoder_name, dataset_name, parent_directory)
    ...
    def save_embeddings(self, encoder_name, dataset_name, parent_directory='../embeddings'):
        ...
        for i in range(self.__len__()):
            try:
                np.save(os.path.join(out_dir, 'embeddings_'+str(i)),
                        self.embeddings[i])
            except TypeError:
                np.save(os.path.join(out_dir, 'embeddings_'+str(i)),
                        self.embeddings[i].detach().cpu())
    ...
    def collater(self, samples):
        ...
        def merge(values, continuous=False):
            if len(values[0].shape) < 2:
                ...
                result = torch.zeros(
                    (len(values), max_length, values[0].shape[1]))
                ...
                if self.mi:
                    for index in range(len(v)):
                        if np.random.rand() < self.mp and not v[index]:
                            # avoiding using the mask where the ignore index argument is not allowed (e.g. BinaryCrossEntropy loss and Conditional Random Fields models)
                            v[index] = v[index]-self.minus
        ...
        if self.precompute:
            ...
            if self.infer:
                ...
                tgt_tokens = merge_tags(
                    [torch.tensor(s['target']) for s in samples])

            if self.cos:
                segments = [[index for index, x in enumerate(
                    s['target']) if x] for s in samples]
            else:
                segments = None
            ...
            if self.encoder2 is not None:
                src_tokens = merge([torch.cat((self.encoder.encode(s['source'], convert_to_tensor=True), self.encoder2.encode(
                    s['source'], convert_to_tensor=True))) for s in samples])
            else:
                src_tokens = merge([self.encoder.encode(
                    s['source'], convert_to_tensor=True) for s in samples])
            if self.infer:
                tgt_tokens = None
            else:
...
class CrossEncoderDataset(Dataset):
    def __init__(self, all_sentences, enc="bert-base-uncased", max_length=512, minus=False, longformer=False):
        ...
        if longformer:
            ...
            self.sentences = [[f' {sep_token} '.join([sentences[0][i], sentences[0][i + 1]]) for i in range(
                len(sentences[0]) - 1)] + [sentences[0][-1]] for sentences in all_sentences]
    ...
    def collater(self, samples):
        ...
        def merge(tags, input_tensor=True, labels=False):
            ...
            if input_tensor:
                if labels:
                    result = torch.zeros(
                        (len(tags), max_length), dtype=data_type) - self.mi
                    ...
                    result = torch.zeros(
                        (len(tags), max_length, max_tokens), dtype=data_type) + self.pad_id
        ...
        for s in src_sentences:
            if len(s) < 1:
                ...
                0/0
        try:
            tokens_and_masks = [self.tokenizer(
                s["source"], padding=True, truncation=True, return_tensors='pt', max_length=self.max_length) for s in samples]
        except IndexError:
            print("this did not work: here are the sentences:")
            print(src_sentences)
            0/0
        src_tokens = merge([t["input_ids"] for t in tokens_and_masks])
        src_attention_masks = merge([t["attention_mask"]
                                    for t in tokens_and_masks])

        src_lengths = torch.LongTensor([len(s['source']) for s in samples])
        tgt_tokens = merge([torch.tensor(s['target'])
                           for s in samples], labels=True)
        segments = None
...
class Predictor:
    ...
    def predict(self, docs, batch=False,
        ...
        if batch:
            ...
            if verbose:
                print('Aggregating {} documents together to process them in batch...'.format(
                    batch_size))
            ...
            for index, batch in enumerate(dl):
                ...
                for batch_index, boundaries in enumerate(batch_boundaries):
                    ...
                    embs = inputs[batch_index][:lengths[batch_index]
                    ...
                    for index, boundary in enumerate(boundaries):
                        if boundary:
                            segments.append(
                                input_sentences[batch_index][last_index:index + 1])
...
class BERT_BASE_ENCODER:
    ...
    def encode(self, sentences, convert_to_tensor=False, batch_size=32):
        ...
        length_sorted_idx = np.argsort(
            [-len(sen.split()) for sen in sentences])
        ...
        if self.opt == "combined":
            ...
            length_sorted_idx_ = np.argsort(
                [-len(sen.split()) for sen in sentences_pr])
            ...
            all_embeddings_pr = [all_embeddings_pr[idx]
                                 for idx in np.argsort(length_sorted_idx)]
...
def get_model(model_name, max_length=None, labels=None, emb_diff=False):
    ...
    if model_name.lower() == 'use_large':
        ...
        assert os.path.exists(
            MODEL_PATH), "You need to download the pre-trained infersent model in order to use it!"
        ...
        assert os.path.exists(
            W2V_PATH), "You need to download the relative GloVe embeddings as specified in the instructions and place the resulting folder in the main directory in order to use this encoder!"
        ...
        if max_length is not None:
            encoder = SentenceTransformer(
                os.path.join("pretrained_encoders", model_name))
        ...
        # The model name for huggingface should be in the format {base_model_name}-{pooling_code}; see the huggingface_models dictionary.
        enc, pool = model_name.split("-")
        ...
        if len(pool) == 2:
            ...
            raise ValueError(
                "You added too many + signs in the model name: the + sign is used to use one of pairwise or combined encoding. Please refrain from using it unless you want one (and only one) of those options!")
        ...
        encoder = BERT_BASE_ENCODER(
            enc, pool, opt=opt, emb_diff=emb_diff, max_length=max_length)
        return (encoder, None)
...
class TopBertEncoder():
    ...
    def encode(self, sentences, convert_to_tensor=True):
        top_emb = self.top_encoder.encode(
            sentences, convert_to_tensor=convert_to_tensor)
        bert_emb = self.bert_encoder.encode(
            sentences, convert_to_tensor=convert_to_tensor)
...
def get_vq_model(model_path):
    ...
    topic_matrix = pd.read_csv(os.path.join(
        model_path, 'topic_matrix.csv'), index_col=False)
...
class WordLevelDataset(Dataset):
    def __init__(self, lines, tag_to_ix, wordToIndex, embedding_layer):
        ...
        self.sentences = [[torch.LongTensor(w2iGLOVE(sentence, wordToIndex)).detach(
        ) for sentence in line[0]] for line in lines]

        for line in lines:
            # new_line = []
            self.sentence_lengths.append(
                [len(w2iGLOVE(x, wordToIndex)) for x in line[0]])
    ...
    def collater(self, samples):
        ...
        def merge(values, max_length=None):
            ...
            if len(values[0].shape) == 3:
                ...
                result = torch.zeros(
                    (len(values), max_length, values[0].shape[1], values[0].shape[2]))
                for i, v in enumerate(values):
                    result[i, :len(v)] = v
                return result
        ...
        id = torch.tensor([s['id'] for s in samples])
        src_tokens = merge([merge([self.embed(x).detach()
                           for x in s['source']], max_words) for s in samples])
        tgt_tokens = merge_tags([torch.tensor(s['target'])
                                for s in samples], ignore_idx=1)
        ...
        for index, l1 in enumerate(sentence_lengths):
            ...
            if l2 > index:
                ...
                new_sentence_length.extend(
                    [1 for x in range(max_src_length-src_lengths[src_index])])
