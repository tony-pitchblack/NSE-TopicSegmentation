    ...
    for b_index, X in enumerate(batch):
        ...
        for i in range(doc_length):
            ...
            if i <= win_size:
                ...
                z = torch.cat([Z[i, :], sigmoid(torch.diag(
                    torch.mm(a_norm, b_norm.transpose(0, 1)))).unsqueeze(-1)], -1)
                ...
                b_norm = b / b.norm(dim=1)[:, None]
                z = torch.cat([Z[win_size, :], sigmoid(torch.diag(
                    torch.mm(a_norm, b_norm.transpose(0, 1)))).unsqueeze(-1)], -1)
                attn_weight = softmax(self_attn(z)).permute(1, 0)

                output = attn_weight.matmul(
                    Z[win_size, :, 0:int(Z.size()[-1]/2)])  # shape: (hidden, 1)
...
def auxiliary_coherence_function(batch, targets, lengths, projection):
    ...
    for i, doc_len in enumerate(lengths):
        ...
        forward_padded_x = projection(batch_x[:-1, :hidden] - F.pad(
            batch_x[:-1, :hidden], (0, 0, 1, 0), 'constant', 0)[:-window, :])
        backward_padded_x = projection(batch_x[1:, hidden:] - F.pad(
            batch_x[1:, hidden:], (0, 0, 0, 1), 'constant', 0)[window:, :]).permute(1, 0)
        sims_outputs.append(F.sigmoid(torch.diag(
            torch.mm(forward_padded_x, backward_padded_x))))
        ...
        for segment_index, seg in enumerate(segment_indeces[batch_index]):
            ...
            if positive:
                ...
                if len(embeddings[prev_seg:seg]) > 1:
                    ...
                    if mean:
                        samples1 = torch.cat(
                            (samples1, first_.mean(0).unsqueeze(0)))
                        samples2 = torch.cat(
                            (samples2, second_.mean(0).unsqueeze(0)))
                    else:
                        samples1 = torch.cat(
                            (samples1, first_.sum(0).unsqueeze(0)))
                        samples2 = torch.cat(
                            (samples2, second_.sum(0).unsqueeze(0)))
            else:

                if mean:
                    samples1 = torch.cat(
                        (samples1, embeddings[prev_seg:seg][:emb_size//2].mean(0).unsqueeze(0)))
                else:
                    samples1 = torch.cat(
                        (samples1, embeddings[prev_seg:seg][:emb_size//2].sum(0).unsqueeze(0)))

                try:

                    next_seg = min(
                        segment_indeces[batch_index][segment_index+1]-seg,  seg + max_len)
...
def cosine_loss(batched_embeddings, sequence_lengths, segment_indeces, cosine_loss_class, device):
    ...
    positives = aggregate_embeddings(
        batched_embeddings, sequence_lengths, segment_indeces, device)
    ...
    targets = torch.cat((targets, torch.tensor(
        [1 for x in range(len(positives[0]))])))

    negatives = aggregate_embeddings(
        batched_embeddings, sequence_lengths, segment_indeces, device, positive=False)

    samples1 = torch.cat((samples1, negatives[0]), axis=0)
    samples2 = torch.cat((samples2, negatives[1]), axis=0)
    targets = torch.cat((targets, torch.tensor(
        [-1 for x in range(len(negatives[0]))])))
    if targets.tolist():
        loss = cosine_loss_class(samples1.to(
            device), samples2.to(device), targets.to(device))
...
def coherence_classification(batched_embeddings, sequence_lengths, segment_indeces, coherence_loss_class, projection, device):
    ...
    targets = torch.cat((targets, torch.tensor(
        [1 for x in range(len(positives))])))
    ...
    targets = torch.cat((targets, torch.tensor(
        [0 for x in range(len(positives))])))
...
class BiLSTM(nn.Module):
    def __init__(self, tagset_size, embedding_dim, hidden_dim, num_layers=1,
        ...
        if add_embedding:
            self.classification = nn.Linear(
                hidden_dim*2 + embedding_dim, self.tagset_size)
        else:
        ...
        if loss_fn == 'CrossEntropy':
            ...
            raise ValueError(
                'Choose one of CrossEntropy or BinaryCrossEntropy as loss function')
    ...
    def loss(self, xs, lengths, tags, segments=None, auxiliary_task=False, use_cosine=True, aggregate_sentences=True, device=None):
        ...
        if self.aux:
            ...
            sim, sim_tags = auxiliary_coherence_function(
                x, tags, lengths, self.aux_projection)
            ...
            if use_cosine:
                coh_loss = cosine_loss(
                    x, lengths, segments, self.cosine_loss, device)
            else:
                coh_loss = coherence_classification(
                    x, lengths, segments, self.coherence_loss, self.coherence_projection, device)
        ...
        if self.rsa:
            x = xing_restricted_attention(
                x, lengths, self.win_size, self.self_attn)
        ...
        if self.add_emb:
            x = self.classification(torch.cat(
                (x, xs), axis=1)) if aggregate_sentences else self.classification(torch.cat((x, xs), axis=2))
        else:
            x = self.classification(x)
        ...
        if self.bce:
            ...
            loss = self.loss_fn(x.reshape(-1, self.tagset_size),
                                tags.reshape(-1).type(torch.LongTensor).to(device))
    ...
    def forward(self, xs, lenghts, threshold=0.4, device=None):
        ...
        if self.rsa:
            x = xing_restricted_attention(
                x, lenghts, self.win_size, self.self_attn)
            if self.model2 is not None:
                _, x = self.model2(x, lenghts)

        if self.add_emb:
            scores = self.classification(torch.cat(
                (x, xs), axis=1)) if aggregate_sentences else self.classification(torch.cat((x, xs), axis=2))
        else:
...
class TransformerCRF(nn.Module):
    def __init__(self, tagset_size, embedding_dim, hidden_dim, num_layers=6,
        ...
        self.model = Transformer(in_dim=embedding_dim, h_dim=hidden_dim, n_heads=nheads, n_layers=num_layers, dropout=dropout_in, drop_out=dropout_out,
                                 batch_first=batch_first, device=self.device, positional_encoding=positional_encoding, restricted=restricted, window_size=window_size)
...
class Transformer_segmenter(nn.Module):
    def __init__(self, tagset_size, embedding_dim, hidden_dim, num_layers=6,
        ...
        if restricted:
            # TODO: disentangle pyramidal transformer from simple restricted one (give option to choose one or the other)
            pyramidal = True
            self.no_mask = True
        ...
        if restricted:
            if pyramidal:
                window_size = [
                    win*window_size for win in range(num_layers, 0, -1)]
        ...
        self.classification = nn.Linear(
            embedding_dim*final_dim_multiplier, self.tagset_size)
...
class TextSeg(nn.Module):
    def __init__(self, tagset_size, embedding_dim, hidden_dim, Embedding_layer=None, num_layers=1,
        ...
        if pooling == 'max':
            ...
            self.feedforward = nn.Sequential(
                nn.Linear(2*hidden, 2*hidden), nn.Tanh())
    ...
    def loss(self, x, sentence_lengths, doc_lengths, tags):
        ...
        if self.attn:
            ...
            x = attention_pooling(x, self.context_vector,
                                  sentence_lengths, doc_lengths)
        else:
        ...
        if self.aux:
            ...
            sim, sim_tags = auxiliary_coherence_function(
                x, tags, doc_lengths, self.aux_projection)

            coh_loss = self.coherence_loss(sim, sim_tags.to(sim.device))

        if self.rsa:
            x = xing_restricted_attention(
                x, doc_lengths, self.win_size, self.self_attn)
            if self.model2 is not None:
...
class MLP(nn.Module):
    ...
    def __init__(self, tagset_size, embedding_dim, hidden_dim, num_layers=1,
        ...
        if num_layers == 0:
            ...
            od = OrderedDict(
                [("proj_0", nn.Linear(embedding_dim, hidden_dim)), ("relu_0", nn.ReLU())])
...
class Pair_MLP(nn.Module):
    ...
    def __init__(self, tagset_size, embedding_dim, hidden_dim, num_layers=1,
        ...
        if num_layers == 0:
            od = OrderedDict([("dropout_in", nn.Dropout(
                p=dropout_in)), ("final_proj", nn.Linear(embedding_dim*3, self.tagset_size))])
        else:
            od = OrderedDict([("dropout_in_0", nn.Dropout(p=dropout_in)), ("proj_0", nn.Linear(
                embedding_dim, hidden_dim)), ("relu_0", nn.ReLU()), ("dropout_out_0", nn.Dropout(p=dropout_out))])
            for i in range(num_layers-1):
    ...
    def loss(self, xs, lengths, tags, segments=None, auxiliary_task=False, use_cosine=True, aggregate_sentences=False, device=None):
        ...
        input_pairs = torch.cat(
            [xs[:, :-1, :], xs[:, 1:, :], torch.abs(xs[:, :-1, :]-xs[:, 1:, :])], axis=2)
    ...
    def forward(self, xs, lenghts, threshold=0.4, device=None):
        ...
        if self.bce:
            scores = torch.cat(
                (scores, torch.ones(bs, 1, 1).to(xs.device)), axis=1)
        else:
            scores = torch.cat(
                (scores, torch.ones(bs, 1, 2).to(xs.device)), axis=1)
...
class SheikhBiLSTM(nn.Module):
    def __init__(self,
        ...
        if loss_fn == 'BinaryCrossEntropy':
            ...
            raise ValueError(
                'Choose one of FocalLoss or BinaryCrossEntropy as loss function')
    ...
    def loss(self, inputs, lengths, tags, device=None, segments=None):
        ...
        if self.fl:
            loss = self.loss_fn(torch.cat(x_unpad).reshape(-1),
                                torch.cat(y_unpad).to(inputs.device), sigmoid=False)
        else:
            loss = self.loss_fn(torch.cat(x_unpad).reshape(-1),
                                torch.cat(y_unpad).to(inputs.device))
        return loss
    ...
    def forward(self, inputs, lenghts, threshold=0.4, device=None):
        ...
        # Add the extra time step corresponding to the last output (will affect just the longest sequence)
        scores = torch.cat(
            (scores, torch.ones(bs, 1, 1).to(inputs.device)), axis=1)
        if self.th is not None:
            threshold = self.th
        scores = 1-self.sigmoid(scores)
        tag_seq = scores[:, :, 0] > threshold
...
class CrossEncoderModel(nn.Module):
    ...
    def first_last_pooling(self, model_output, attention_mask):
        input_mask_expanded = attention_mask.unsqueeze(
            -1).expand(model_output[-1].size()).float()
    ...
    def second_to_last_pooling(self, model_output, attention_mask):
        input_mask_expanded = attention_mask.unsqueeze(
            -1).expand(model_output[-2].size()).float()
        ...
        for i, x_ in enumerate(xs):
            ...
            if seq_len > self.max_seq_len:
                ...
                for idx in range(seq_len//self.max_seq_len):
                    x_tmp.append(self.bert(input_ids=x_[self.max_seq_len*idx:self.max_seq_len*(
                        idx+1)], attention_mask=attention_mask[i, self.max_seq_len*idx:self.max_seq_len*(idx+1)], output_hidden_states=True).hidden_states)
                if seq_len % self.max_seq_len:
                    x_tmp.append(self.bert(input_ids=x_[self.max_seq_len*(idx+1):seq_len], attention_mask=attention_mask[i, self.max_seq_len*(
                        idx+1):seq_len], output_hidden_states=True).hidden_states)
                x_ = [torch.cat([e[i] for e in x_tmp], axis=0)
                      for i in range(len(x_tmp[0]))]
            else:
                x_ = self.bert(
                    input_ids=x_, attention_mask=attention_mask[i, :lengths[i]], output_hidden_states=True).hidden_states
            # print(x_[-1].shape)
            ...
            if seq_len > self.max_seq_len:
                ...
                for idx in range(seq_len//self.max_seq_len):
                    x_tmp.append(self.bert(input_ids=x_[self.max_seq_len*idx:self.max_seq_len*(
                        idx+1)], attention_mask=attention_mask[i, self.max_seq_len*idx:self.max_seq_len*(idx+1)], output_hidden_states=True).hidden_states)
                if seq_len % self.max_seq_len:
                    x_tmp.append(self.bert(input_ids=x_[
                                 self.max_seq_len*(idx+1):], attention_mask=attention_mask[i, self.max_seq_len*(idx+1):], output_hidden_states=True).hidden_states)
